    ...
    fun `given initially allowed to show, initial state is allowed to show`() =
        runTest {
            moleculeFlow(RecompositionMode.Immediate) {
                ...
                assertEquals(ProveYourIdentityModalState(allowedToShow = true), awaitItem())
            }
        }

    @Test
    fun `given not initially allowed to show, initial state is allowed to show`() =
        runTest {
            moleculeFlow(RecompositionMode.Immediate) {
                rememberProveYourIdentityModalState(
                ...
                assertEquals(ProveYourIdentityModalState(allowedToShow = false), awaitItem())
            }
        }
